...
import type { AgentConfigContribution, CommandContribution } from "./config-contribution.js";
...
export async function loadAgentDefinitions(assetsDir: string): Promise<LoadedAgent[]> {
...
export async function loadCommand(assetsDir: string): Promise<CommandContribution | undefined> {
  try {
    const raw = await readFile(join(assetsDir, "commands", "croupier.md"), "utf8");
    const { data, body } = frontmatter(raw);
